...
class MassNotApproved extends \Magento\Backend\App\Action implements HttpPostActionInterface
{
    /**
     * @see _isAllowed()
     */
    ...
    public function execute()
    {
        ...
        foreach ($collection as $job_application) {
            $result = $this->answerFactory->create()->load($job_application->getAnswerId());
            $status=1;
            $result->setStatus($status);
            $result->save();
        }
        $this->messageManager->addSuccessMessage(__(
            'A total of %1 record(s) have been Not Approved.',
            $collectionSize
        ));
